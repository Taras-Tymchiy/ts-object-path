import { createProxy, isProxy, getPath, getPlainPath, get, set } from '../src/ts-object-path'
...
describe('getPlainPath test', () => {
  it('Creates proxy with empty path', () => {
    const p = createProxy<ITest>()
    expect(getPlainPath(p)).toEqual([])
  })
  it('Gets path from proxy', () => {
    const p = createProxy<ITest>()
  ...
  it('Get undefined', () => {
    expect(getPlainPath({})).toBeUndefined()
  })
})
