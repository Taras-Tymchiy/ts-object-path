import { ObjPath, ObjPathProxy } from './obj-path-proxy'
...
export function createProxy<T extends ObjPath>(path: PropertyKey[] = []): ObjPathProxy<T, T> {
...
export function getPath<TRoot extends ObjPath, T>(proxy: ObjProxyArg<TRoot, T>): PropertyKey[] {
...
export function isProxy<TRoot extends ObjPath, T>(value: any): value is ObjPathProxy<TRoot, T> {
  return value && typeof value === 'object' && !!getPath<TRoot, T>(value as ObjPathProxy<TRoot, T>)
}

export function get<TRoot extends ObjPath, T>(
  object: TRoot,
...
export function set<TRoot extends ObjPath, T>(
  object: TRoot,
  proxy: ObjProxyArg<TRoot, T>,
  value: T
): void {
